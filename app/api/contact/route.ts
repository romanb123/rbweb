...
export async function POST(req: NextRequest) {
  try {
    ...
    await resend.emails.send({
      ...
      subject: `RBapp — הודעה חדשה מ-${name}`,
      replyTo: email,
      html: `
        <div style="font-family:sans-serif;max-width:580px;margin:0 auto;background:#fff;">
          <div style="background:#060606;padding:24px 28px;display:flex;align-items:center;gap:12px;">
            ...
            <span style="color:#f0f0f0;font-weight:700;font-size:18px;letter-spacing:-0.04em;">RBapp</span>
          </div>
          ...
          <div style="padding:16px 28px;background:#f3f3f3;">
            <p style="margin:0;font-size:12px;color:#aaa;">נשלח מ-rbapp.amplifyapp.com</p>
          </div>
        </div>
      `,
    })
